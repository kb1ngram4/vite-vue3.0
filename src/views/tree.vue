<template>
  <div>
    <el-tree ref="treeRef" node-key="id" style="max-width: 600px" :props="props" :load="loadNode" lazy show-checkbox
      :default-checked-keys="checkedKeys" :default-expanded-keys="expandedKeys" :getCheckedKeys="getCheckedKeys"
      :getCurrentKey="getCurrentKey" @check="checkChange" />
  </div>
</template>
...
import { ref } from 'vue';
const checkedKeys = ref<string[]>([]); // 需要回显的选中节点的 key 数组
const expandedKeys = ref<string[]>([]); // 需要回显的展开节点的 key 数组
...
const checkChange = (data: any, checkedNodes: any, checkedKeys: any, halfCheckedNodes: any, halfCheckedKeys: any) => {
  console.log('checkChange', data, checkedNodes, checkedKeys, treeRef.value);
  // 获取选中的节点
  // const checkedKeys = treeRef.value.getCheckedKeys();
  //   const halfCheckedKeys = treeRef.value.getHalfCheckedKeys();
}
const getCurrentKey = (val: any) => {
  console.log('getCurrentKey', val);
}
const getCheckedKeys = (val: any) => {
...
const fetchChildOptions = (parentId: number) => {
  return new Promise((resolve) => {
